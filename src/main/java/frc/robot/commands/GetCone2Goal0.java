...
import frc.robot.Constants.GetCone2Goal0Constants;
...
public class GetCone2Goal0 extends CommandBase {
  ...
  public void execute() {
    ...
    if (timer.advanceIfElapsed(GetCone2Goal0Constants.fordwardTime1) == true && step == 1){
      speed = 0;
      rspeed = 0;
      step+=1;
    }
    if (step == 2){
      rspeed = GetCone2Goal0Constants.rspeed1;
      timer.reset();
      step+=1;
    }
    if (step == 3 && timer.advanceIfElapsed(GetCone2Goal0Constants.rotateTime1) == true){
      step+=1;
      rspeed = 0;
    }
    if (step == 4){
    ...
    if (step  == 5 && timer.advanceIfElapsed(GetCone2Goal0Constants.releaseTime1) == true){
      step+=1;
    }
